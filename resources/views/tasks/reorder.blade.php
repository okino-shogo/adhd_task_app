...
@section('content')
    <div class="container mx-auto p-6 max-w-2xl">
        <!-- 戻るボタン -->
        <div class="mb-6">
            <a href="{{ route('tasks.index') }}" class="bg-blue-500 text-black px-4 py-2 rounded shadow-md hover:bg-blue-600 transition duration-200">
                ← 戻る
            </a>
        </div>
        ...
        <div class="bg-white shadow-lg rounded-lg p-6">
            ...
            <ul id="task-list" class="space-y-4">
                @foreach ($tasks as $task)
                    <li class="flex items-center bg-gray-100 p-4 rounded shadow-md task-item" data-id="{{ $task->id }}" data-priority="{{ $task->priority }}" data-status="{{ $task->status }}">
                        ...
                        <div class="flex-grow ml-4 text-lg font-semibold">
                            <input type="text" value="{{ $task->task_name }}" class="task-name bg-transparent border-b-2">
                            <input type="time" value="{{ $task->deadline }}" class="task-deadline bg-transparent border-b-2">
                            <input type="number" value="{{ $task->importance }}" class="task-importance bg-transparent border-b-2" min="1" max="5">
                            <input type="time" value="{{ $task->estimated_time }}" class="estimated_time bg-transparent border-b-2" min="1">
                        ...
                        <button class="update-task bg-blue-500 text-white px-4 py-2 rounded ml-2" data-id="{{ $task->id }}">更新</button>
                        <button class="delete-task bg-red-500 text-white px-4 py-2 rounded ml-2" data-id="{{ $task->id }}">削除</button>
                    </li>
                @endforeach
            </ul>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('save-order').addEventListener('click', function() {
                let order = [];
                document.querySelectorAll('.task-item').forEach((item, index) => {
                    order.push({ id: item.getAttribute("data-id"), priority: index + 1 });
                });
